...
import { 
  ChevronLeft, Star, Heart, ShoppingCart, Share2, Eye, 
  ThumbsUp, ThumbsDown, MessageCircle, Send, User, 
  ChevronRight, Check, Package, Shield, Zap
} from 'lucide-react';
...
const ProductDetailPage = () => {
  ...
  if (loading) {
    return (
      <div className="dark-bg" style={{ minHeight: '100vh', paddingTop: '6rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
    ...
    return (
      <div className="dark-bg" style={{ minHeight: '100vh', paddingTop: '6rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <button onClick={() => navigate('/marketplace')} className="glass-subtle" style={{ padding: '1rem 2rem', borderRadius: '12px', border: 'none', cursor: 'pointer' }}>
            Back to Marketplace
          </button>
        </div>
      </div>
  ...
  const currentImage = images[selectedImage] || images[0] || { url: 'https://via.placeholder.com/600' };
      ...
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 2rem' }}>
        ...
        <div style={{
              ...
              <div style={{
                ...
                {images.map((img, idx) => (
                  <button
                    ...
                    style={{
                      border: selectedImage === idx ? '2px solid rgba(255, 255, 255, 0.4)' : '1px solid rgba(255, 255, 255, 0.1)',
                      borderRadius: '12px',
                      overflow: 'hidden',
                      cursor: 'pointer',
                      paddingTop: '100%',
          ...
          <div>
            ...
            <div style={{
              ...
              {product.average_rating > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontWeight: '700', fontSize: '1.125rem' }}>{product.average_rating.toFixed(1)}</span>
                </div>
              )}
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: '600', fontSize: '0.9375rem' }}>
                Quantity
              </label>
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '1.25rem', fontWeight: '700', minWidth: '40px', textAlign: 'center' }}>
                  {quantity}
                </span>
                <button
            ...
            <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
              ...
              >
                <Heart size={22} color="#fff" fill={user && product.wishlist_count > 0 ? '#ff3b30' : 'none'} />
              </button>
        ...
        <div className="glass-strong" style={{
          ...
          <div style={{ padding: '2rem' }}>
            {activeTab === 'overview' && (
              <div>
                <h3 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: '1.5rem' }}>Product Overview</h3>
                <p style={{ fontSize: '1.0625rem', lineHeight: '1.8', opacity: 0.9, marginBottom: '2rem' }}>
                  {product.description}
                </p>
                
                {product.specifications && product.specifications.length > 0 && (
                  <>
                    <h4 style={{ fontSize: '1.25rem', fontWeight: '700', marginBottom: '1rem' }}>Key Features</h4>
            ...
            {activeTab === 'specs' && (
              <div>
                <h3 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: '1.5rem' }}>Technical Specifications</h3>
                {product.specifications && product.specifications.length > 0 ? (
                  <div className="glass-subtle" style={{
                    borderRadius: '12px',
                    border: '1px solid rgba(255, 255, 255, 0.1)',
                    ...
                    {product.specifications.map((spec, idx) => (
                      <div 
                        ...
                        style={{
                          padding: '1.25rem 1.5rem',
                          borderBottom: idx < product.specifications.length - 1 ? '1px solid rgba(255, 255, 255, 0.05)' : 'none',
                          display: 'grid',
            ...
            {activeTab === 'chat' && (
              <ChatTab product={product} token={token} />
            )}
          </div>
        </div>
      </div>
...
const ReviewsTab = ({ reviews, product, ratingDistribution, onSubmitReview, newReview, setNewReview, token }) => {
  return (
    <div>
      <h3 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: '2rem' }}>Customer Reviews</h3>
      ...
      {token && (
        <div className="glass-subtle" style={{
          ...
          <h4 style={{ fontSize: '1.25rem', fontWeight: '700', marginBottom: '1.5rem' }}>Write a Review</h4>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                ...
                <div>
                  <div style={{ fontWeight: '700', marginBottom: '0.25rem' }}>{review.username || 'Anonymous'}</div>
                  <div style={{ display: 'flex', gap: '0.25rem' }}>
                    {[...Array(5)].map((_, i) => (
                      <Star 
                        key={i}
...
const QATab = ({ questions, onSubmitQuestion, newQuestion, setNewQuestion, token }) => {
  return (
    <div>
      <h3 style={{ fontSize: '1.5rem', fontWeight: '700', marginBottom: '2rem' }}>Questions & Answers</h3>
      ...
      {token && (
        <div className="glass-subtle" style={{
          ...
          <h4 style={{ fontSize: '1.125rem', fontWeight: '700', marginBottom: '1rem' }}>Ask a Question</h4>
          <div style={{ display: 'flex', gap: '1rem' }}>
            <input
              type="text"
              value={newQuestion}
        ...
        {questions.length > 0 ? questions.map((question) => (
          ...
          >
            ...
            <div style={{ marginBottom: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'start', gap: '0.75rem', marginBottom: '0.75rem' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '0.8125rem', opacity: 0.7 }}>
                    Asked by {question.username || 'Anonymous'} on {new Date(question.created_at).toLocaleDateString()}
                  </div>
                </div>
              </div>
            </div>
            ...
            {question.answers && question.answers.length > 0 && (
              <div style={{ marginLeft: '2rem', borderLeft: '2px solid rgba(255, 255, 255, 0.1)', paddingLeft: '1rem' }}>
                {question.answers.map((answer, idx) => (
                  <div key={idx} style={{ marginBottom: idx < question.answers.length - 1 ? '1rem' : 0 }}>
            ...
            {(!question.answers || question.answers.length === 0) && (
              <div style={{ marginLeft: '2rem', opacity: 0.6, fontStyle: 'italic', fontSize: '0.875rem' }}>
                No answers yet
              </div>
            )}
          </div>
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        💬 <strong>Coming Soon:</strong> Real-time chat with WebSocket, AI-powered responses (DeepSeek v3), and global chat widget with notifications!
      </div>
    </div>
  );
};
